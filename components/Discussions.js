...
export default function Discussions() {
  ...
  async function loadThreads() {
    const { data } = await supabase.from('team_discussions')
  ...
  async function selectThread(thread) {
    setSelected(thread)
    const { data } = await supabase.from('team_discussion_replies')
  ...
  async function createThread(e) {
    ...
    const { data } = await supabase.from('team_discussions')
      ...
      .select().single()
    await supabase.from('team_activity').insert({ member_id: user.id, member_name: user.name, action: 'posted_discussion', detail: `Started discussion: ${newThread.title}` })
  ...
  async function postReply(e) {
    ...
    await supabase.from('team_discussion_replies').insert({ discussion_id: selected.id, member_id: user.id, content: replyText })
    await supabase.from('team_activity').insert({ member_id: user.id, member_name: user.name, action: 'posted_reply', detail: `Replied in: ${selected.title}` })
    setReplyText('')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl font-light" style={{ color: '#0C1F3F' }}>Discussions</h1>
        ...
        <button onClick={() => { setShowNew(true); setSelected(null) }}
                className="px-4 py-2.5 rounded-xl text-white text-sm font-bold" style={{ background: '#00A8A8' }}>
          + New Thread
        </button>
      </div>
      ...
      {showNew && (
        <form onSubmit={createThread} className="bg-white rounded-xl border border-gray-200 p-6 mb-6 space-y-4">
          <input type="text" required placeholder="Thread title" value={newThread.title}
          ...
          <textarea placeholder="Opening message..." rows={3} value={newThread.topic}
          ...
          <div className="flex gap-2">
            <button type="submit" className="px-5 py-2.5 rounded-xl text-white text-sm font-bold" style={{ background: '#00A8A8' }}>Post</button>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-1 space-y-2">
          {threads.map(t => (
            <button key={t.id} onClick={() => { selectThread(t); setShowNew(false) }}
              ...
              <p className="font-bold text-sm line-clamp-1" style={{ color: '#0C1F3F' }}>{t.title}</p>
              <p className="text-xs text-gray-400 mt-1">
                {t.author?.name || 'Unknown'} &middot; {new Date(t.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
              </p>
            </button>
          ))}
        ...
        <div className="md:col-span-2">
          {selected ? (
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="font-bold text-lg mb-1" style={{ color: '#0C1F3F' }}>{selected.title}</h2>
              <p className="text-xs text-gray-400 mb-4">
                Started by {selected.author?.name} &middot; {new Date(selected.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
              </p>
              {selected.topic && (
                  ...
                  <div key={r.id} className="flex gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-sm font-bold" style={{ color: '#0C1F3F' }}>{r.author?.name}</p>
              ...
              <form onSubmit={postReply} className="flex gap-2">
                ...
                <button type="submit" className="px-4 py-3 rounded-xl text-white text-sm font-bold shrink-0" style={{ background: '#00A8A8' }}>Reply</button>
              </form>
            </div>
          ) : (
