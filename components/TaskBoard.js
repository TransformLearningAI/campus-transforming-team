...
export default function TaskBoard() {
  ...
  const [newTask, setNewTask] = useState({ title: '', description: '', category: TASK_CATEGORIES[0], priority: 'medium', due_date: '' })

  useEffect(() => { loadTasks() }, [filter])

  async function loadTasks() {
    let q = supabase.from('team_tasks').select('*, claimed:team_members!team_tasks_claimed_by_fkey(name)').order('created_at', { ascending: false })
  ...
  async function createTask(e) {
    e.preventDefault()
    await supabase.from('team_tasks').insert({ ...newTask, created_by: user.id, due_date: newTask.due_date || null })
    await supabase.from('team_activity').insert({ member_id: user.id, member_name: user.name, action: 'created_task', detail: `Created task: ${newTask.title}` })
    setNewTask({ title: '', description: '', category: TASK_CATEGORIES[0], priority: 'medium', due_date: '' })
    setShowNew(false)
    loadTasks()
  }

  async function claimTask(taskId) {
    await supabase.from('team_tasks').update({ claimed_by: user.id, claimed_at: new Date().toISOString(), status: 'in_progress' }).eq('id', taskId)
    await supabase.from('team_activity').insert({ member_id: user.id, member_name: user.name, action: 'claimed_task', detail: `Claimed a task` })
    loadTasks()
  }

  async function completeTask(taskId) {
    await supabase.from('team_tasks').update({ status: 'completed', completed_at: new Date().toISOString() }).eq('id', taskId)
    await supabase.from('team_activity').insert({ member_id: user.id, member_name: user.name, action: 'completed_task', detail: `Completed a task` })
    loadTasks()
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl font-light" style={{ color: '#0C1F3F' }}>Task Board</h1>
      ...
      {showNew && (
        <form onSubmit={createTask} className="bg-white rounded-xl border border-gray-200 p-6 mb-6 space-y-4">
          ...
          <textarea placeholder="Description (optional)" value={newTask.description} rows={3}
          ...
          <div className="flex gap-2">
            <button type="submit" className="px-5 py-2.5 rounded-xl text-white text-sm font-bold" style={{ background: '#00A8A8' }}>Create Task</button>
          ...
          <div key={task.id} className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                <h3 className="font-bold text-sm" style={{ color: '#0C1F3F' }}>{task.title}</h3>
                {task.description && <p className="text-xs text-gray-500 mt-1 line-clamp-2">{task.description}</p>}
                {task.claimed && task.claimed.name && (
                  <p className="text-xs mt-2" style={{ color: '#00A8A8' }}>Claimed by {task.claimed.name}</p>
                )}
              </div>
              <div className="flex gap-2 shrink-0">
                {task.status === 'open' && (
                  <button onClick={() => claimTask(task.id)}
                          className="px-3 py-1.5 rounded-lg text-xs font-bold text-white" style={{ background: '#00A8A8' }}>
                    Claim
                  </button>
                )}
