...
export default function DocumentsHub() {
  ...
  async function loadDocs() {
    const { data } = await supabase
      .from('team_documents')
      .select('*, creator:team_members!team_documents_created_by_fkey(name)')
  ...
  async function addDoc(e) {
    ...
    setErr('')
    setSaving(true)
    const { error } = await supabase.from('team_documents').insert({ ...form, created_by: user.id })
    if (error) {
      setErr('Could not add the link. Please try again.')
      setSaving(false)
      return
    }
