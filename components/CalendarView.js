...
export default function CalendarView() {
  ...
  async function loadEvents() {
    const { data } = await supabase.from('team_events').select('*, creator:team_members!team_events_created_by_fkey(name)')
  ...
  async function createEvent(e) {
    e.preventDefault()
    await supabase.from('team_events').insert({ ...form, created_by: user.id })
    await supabase.from('team_activity').insert({ member_id: user.id, member_name: user.name, action: 'created_event', detail: `Scheduled: ${form.title}` })
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl font-light" style={{ color: '#0C1F3F' }}>Calendar</h1>
        ...
        <form onSubmit={createEvent} className="bg-white rounded-xl border border-gray-200 p-6 mb-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="datetime-local" required value={form.start_time} onChange={e => setForm(f => ({ ...f, start_time: e.target.value }))}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <input type="url" placeholder="Meeting URL (Zoom, Meet)" value={form.meeting_url} onChange={e => setForm(f => ({ ...f, meeting_url: e.target.value }))}
                   className="px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none" />
          </div>
          <div className="flex gap-2">
            <button type="submit" className="px-5 py-2.5 rounded-xl text-white text-sm font-bold" style={{ background: '#00A8A8' }}>Create Event</button>
            ...
            <div className="flex items-start gap-4">
              <div className="text-center shrink-0 w-14">
                <p className="text-xs font-bold uppercase text-gray-400">
                  {new Date(ev.start_time).toLocaleDateString('en-US', { month: 'short' })}
                </p>
              ...
              <div className="flex-1">
                <h3 className="font-bold text-sm" style={{ color: '#0C1F3F' }}>{ev.title}</h3>
                ...
                <div className="flex gap-3 mt-2">
                  ...
                  {ev.meeting_url && <a href={ev.meeting_url} target="_blank" rel="noopener noreferrer" className="text-xs font-medium" style={{ color: '#00A8A8' }}>Join Meeting →</a>}
                </div>
              </div>
            </div>
          </div>
