            ...
            <div key={c.id} className="flex gap-2.5">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-xs font-bold" style={{ color: '#0C1F3F' }}>
                    {c.author?.name}
                  </p>
          ...
          <div className="flex items-start justify-between gap-3 mb-2">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span
                  className={`text-[10px] font-bold ${isToday ? 'text-[#00A8A8]' : isPast ? 'text-gray-400' : 'text-gray-500'}`}
                >
                  {relativeDate(item.date)}
                </span>
              </div>
              <h3
                className={`font-bold text-sm ${item.completed ? 'line-through text-gray-400' : ''}`}
                style={{ color: item.completed ? undefined : '#0C1F3F' }}
              >
                {item.title}
              </h3>
            </div>
...
export default function Timeline() {
  ...
  async function createItem(e) {
    ...
    if (error) {
      setErr('Could not add the item. Please try again.')
      setSaving(false)
      return
    }
