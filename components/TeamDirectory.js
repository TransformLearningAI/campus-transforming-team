...
export default function TeamDirectory() {
  ...
  return (
    <div>
      <h1 className="font-serif text-2xl font-light mb-1" style={{ color: '#0C1F3F' }}>Team Directory</h1>
      <p className="text-sm text-gray-400 mb-6">{members.length} members</p>

      <input type="text" placeholder="Search by name, location, or area of interest..."
        ...
        {filtered.map(m => (
          <div key={m.id} className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="font-bold text-sm" style={{ color: '#0C1F3F' }}>{m.name}</p>
            ...
            {(m.areas_of_interest || []).length > 0 && (
              <div className="flex flex-wrap gap-1 mb-3">
                {m.areas_of_interest.slice(0, 4).map(a => (
                  <span key={a} className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 font-medium">{a}</span>
                ))}
                {m.areas_of_interest.length > 4 && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-400">+{m.areas_of_interest.length - 4}</span>
                )}
              </div>
            )}
            <div className="flex gap-2">
              {m.linkedin_url && (
                <a href={m.linkedin_url} target="_blank" rel="noopener noreferrer"
                   className="text-xs font-medium" style={{ color: '#00A8A8' }}>LinkedIn</a>
              )}
              {m.portfolio_url && (
                <a href={m.portfolio_url} target="_blank" rel="noopener noreferrer"
                   className="text-xs font-medium" style={{ color: '#00A8A8' }}>Portfolio</a>
              )}
            </div>
          </div>
        ))}
